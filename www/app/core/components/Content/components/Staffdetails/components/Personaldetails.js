...
class InformationPart extends React.Component{
   ...
   render(){
    let item = this.props.core.Staffdetails;
     let arr = [
{'title':'基本信息','content':[
...
{'title':'联系方式','content':[{'k':'个人手机','value':'personalmobile','v':item.personalmobile},{'k':'个人邮箱','value':'mail','v':item.mail},{'k':'QQ','value':'qq','v':item.qq},{'k':'微信','value':'wx','v':item.wx},{'k':'现居地','value':'currentaddress','v':item.currentaddress},]}
]


   return <div>
          {arr.map((item,index)=>{
              return <InformationPartSingle key={index} item={item}></InformationPartSingle>
              })}
          </div>
        }
}
